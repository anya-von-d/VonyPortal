            ...
            <nav style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {(() => {
                ...
                const groups = [
                  ...
                  { heading: 'Document Center', items: [
                    { label: 'Documents', to: createPageUrl("LoanAgreements") },
                    { label: 'Learn', to: createPageUrl("ComingSoon") },
                  ]},
                ];
                const currentPath = window.location.pathname;
                return groups.map(({ heading, items }, gi) => (
                  <React.Fragment key={heading}>
                    <div style={{ paddingLeft: 12, paddingTop: gi === 0 ? 0 : 10, marginBottom: 4 }}>
                      <span style={{ fontSize: 9, fontWeight: 700, color: '#787776', letterSpacing: '0.1em', textTransform: 'uppercase', fontFamily: "'DM Sans', sans-serif" }}>{heading}</span>
                    </div>
                    {items.map(({ label, to }) => {
