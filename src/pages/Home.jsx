      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '0 28px 64px', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ marginTop: 36 }}>
          <div className="home-top-row" style={{ display: 'grid', gridTemplateColumns: '1.75fr 1fr', columnGap: 16, rowGap: 16, alignItems: 'start' }}>
            ...
            {/* Loan Progress + Upcoming Payments */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16, gridRow: '1 / 2' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
                    <div style={{ textAlign: 'center', padding: '0 12px' }}>
                      ...
                      <div style={{ position: 'relative', width: 80, height: 80, margin: '0 auto' }}>
                        <svg viewBox="0 0 100 100" style={{ transform: 'rotate(-90deg)', width: 80, height: 80 }}>
                          <circle cx="50" cy="50" r="42" fill="none" stroke="rgba(103,138,251,0.15)" strokeWidth="8" />
                          <circle cx="50" cy="50" r="42" fill="none" stroke="#678AFB" strokeWidth="8" strokeLinecap="round" strokeDasharray={2 * Math.PI * 42} strokeDashoffset={2 * Math.PI * 42 * (1 - percentRepaid / 100)} style={{ transition: 'stroke-dashoffset 1s cubic-bezier(0.4, 0, 0.2, 1)' }} />
                        </svg>
                      ...
                      <div style={{ fontSize: 10, color: '#787776', marginTop: 6, whiteSpace: 'nowrap' }}>{formatMoney(totalRepaid)} of {formatMoney(totalLentAmount)}</div>
                      <div style={{ fontSize: 10, color: '#787776', marginTop: 1 }}>repaid</div>
                    </div>
                    <div style={{ textAlign: 'center', padding: '0 12px' }}>
                      <div style={{ fontSize: 11, color: '#787776', marginBottom: 8 }}>Borrowing</div>
                      <div style={{ position: 'relative', width: 80, height: 80, margin: '0 auto' }}>
                        <svg viewBox="0 0 100 100" style={{ transform: 'rotate(-90deg)', width: 80, height: 80 }}>
                          <circle cx="50" cy="50" r="42" fill="none" stroke="rgba(167,157,234,0.15)" strokeWidth="8" />
                          <circle cx="50" cy="50" r="42" fill="none" stroke="#A79DEA" strokeWidth="8" strokeLinecap="round" strokeDasharray={2 * Math.PI * 42} strokeDashoffset={2 * Math.PI * 42 * (1 - percentPaid / 100)} style={{ transition: 'stroke-dashoffset 1s cubic-bezier(0.4, 0, 0.2, 1)' }} />
                        </svg>
                        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                          <div style={{ fontSize: 15, fontWeight: 700, color: '#292827', letterSpacing: '-0.03em', lineHeight: 1 }}>{percentPaid}%</div>
                        </div>
                      </div>
                      <div style={{ fontSize: 10, color: '#787776', marginTop: 6, whiteSpace: 'nowrap' }}>{formatMoney(totalPaidBack)} of {formatMoney(totalBorrowedAmount)}</div>
                      <div style={{ fontSize: 10, color: '#787776', marginTop: 1 }}>paid back</div>
                    </div>
                  </div>
                </div>
              </div>
